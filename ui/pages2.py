...
async def page_reports() -> None:
    ...
    with container:
        ...
        with ui.element("div").classes("report-grid").style("display:grid;grid-template-columns:repeat(3,minmax(280px,1fr));gap:16px;width:100%;align-items:stretch"):
            with ui.element("div").classes("report-action-card").style("border:1px solid rgba(255,255,255,.10);background:rgba(15,23,42,.72);border-radius:16px;padding:16px;min-height:245px"):
                ...
                range_select = ui.select(options=report_range_options, value="Since Risk Fix").props("outlined dense").style("width:230px")
                ...
                with ui.row().classes("items-center").style("gap:10px;margin-top:10px;flex-wrap:wrap"):
                    from_input = ui.input(label="From Date", placeholder="YYYY-MM-DD").props("outlined dense type=date").style("width:170px")
                    to_input = ui.input(label="To Date", placeholder="YYYY-MM-DD").props("outlined dense type=date").style("width:170px")
                ...
                async def _preview_main() -> None:
                    preview_area.clear()
                    preview_area.set_visibility(True)
                    html = await _build_main_report_preview_html(limit_select.value, _range_payload())
                    with preview_area:
                        ui.html(html)
            ...
            with ui.element("div").classes("report-action-card").style("border:1px solid rgba(255,255,255,.10);background:rgba(15,23,42,.72);border-radius:16px;padding:16px;min-height:245px"):
                ...
                analytics_period = ui.select(options=["7d", "30d", "90d", "All"], value="30d").props("outlined dense").style("width:160px")
                ...
                async def _preview_analytics() -> None:
                    preview_area.clear()
                    html = await _build_analytics_report_preview_html(analytics_period.value)
                    with preview_area:
                        ui.html(html)
        ...
        preview_area = ui.element("div").classes("reports-preview-area")
        html = await _build_main_report_preview_html(_report_trade_limit, {"mode": "Since Risk Fix"})
        with preview_area:
            ui.html(html)
